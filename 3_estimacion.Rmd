...
Un parámetro es una función sobre el universo y sus variables, lo denotamos por $\theta$

$$\theta=f(U,X)$$
Un estimador se construye a partir de la definición de una *estadística* ($\Theta$) y tiene el objetivo de aproximar de la mejor forma a un parámetro. 
...
Para este ejercicio, la media muestral es insesgado y más eficiente que la mediana muestral, como un estimador de la media poblacional.
...
$$P_1-P_2$$
  - El cociente de varianzas de dos poblaciones 
...
$$\bar{x}=\frac{\sum_s X_i}{n}$$
  - Para $\sigma^2$, la estimación es la varianza muestral $S^2$, 
...
$$\hat{P}_A=\frac{\#_sA}{n}=\frac{\sum_{s} X_i}{n}; \quad \{X_i=1, \quad i \in A, X_i=0,\quad eoc\}$$
  - Para $\mu_1-\mu_2$, la diferencia entre las medias de las muestras de dos muestras aleatorias *independientes*. 

$$\hat{\mu_1}-\hat{\mu}_2=\bar{X}_1-\bar{X}_2$$
  - Para $P_1-P_2$, la diferencia entre las proporciones de las muestras de dos muestras aleatorias *independientes*.
  
$$\hat{P_1}-\hat{P}_2$$
  - Para el cociente de varianzas $\sigma_1/\sigma_2$ un estimador razonables es:
...
$$=\frac{1}{n-1}E\left[\sum_s X_i^2-2\bar{X}n\frac{\sum_s X_i}{n}  +n\bar{X}^2 \right]=$$
$$=\frac{1}{n-1}E\left[\sum_s X_i^2-2n\bar{X}^2  +n\bar{X}^2 \right]=\frac{1}{n-1}E\left[\sum_s X_i^2-n\bar{X}^2\right]=$$
...
$$\alpha=\frac{1}{n-1}\left[\sum_s (\sigma^2+\mu^2) -n \left(\frac{\sigma^2}{n}+\mu^2 \right) \right]=$$
$$=\frac{1}{n-1}\left[ n \sigma^2+n\mu^2 -\sigma^2-n\mu^2  \right]=\frac{\sigma^2(n-1)}{n-1}=\sigma^2$$
...
Tarea: 
  - Para la varianza muestral $S^2$, encontrar la cota de cramer Rao, suponiendo media conocida y distribución normal. 
  - Suponer que $x\sim exp(\lambda)$, si el objetivo es estimar $\lambda$ encontrar la cota de cramer Rao para $\lambda$.
...
Sea $X_1, X_2, \ldots ,X_n$ una muestra aleatoria de tamaño $n$ de una
...
$$\mu_1=E[x]=m_1=\frac{\sum_s x_i}{n}$$
$$\mu_2=E[x^2]=m_2=\frac{\sum_s x_i^2}{n}$$
...
muestreo de $X$ de la media muestral $\bar{x}$. 
...
Teniendo a $Z\sim N(0,1)$, para armar el intervalo de confianza basta con trabajar
sobre:
...
Algunos textos como el texto guía
...
$Z_{\alpha/2}=1.96$, para el valor de $\sigma$ para el calculo del tamaño de muestra se realizó una piloto en la materia de estadística II a 13 estudiantes y el resultado fue $\sigma=2.72$ (varianza muestral). Así:
...
Ejercicio, Se quiere hacer una encuesta sobre los ingresos de los hogares en un determinado barrio de La Paz, tomar como referencia un nivel de confiabilidad del 95% y 99%, un margen de error de 800 Bs. Suponiendo que $\sigma=1300$.

  * 95%: $10.14\approx 11$
  * 99%: $17.58\approx 18$
  
### Simulación sobre la confiabilidad de los intervalos de confianza.
...
$$P(-Z_{\alpha/2} \leq Z \leq Z_{\alpha/2})=1-\alpha$$
Así, el limite inferior y superior esta dado por:
...
ambas clases de largueros. Encuentre intervalos de confianza al 90%,
95%, 99% y 80% de confiabilidad para la diferencia de estas medias.
...
$$IC_{99\%}(\mu_1-\mu_2): 85.9-73.3 \pm 2.58*\sqrt{\frac{1^2}{18}+\frac{1.5^2}{16}}=[11.45 \quad 13.74]$$
$$IC_{80\%}(\mu_1-\mu_2): 85.9-73.3 \pm 1.28*\sqrt{\frac{1^2}{18}+\frac{1.5^2}{16}}=[12.03 \quad 13.17]$$

### Intervalo de confianza para la media y la diferencia de medias con *varianza desconocida* pero muestra mayor a 30.

Para los 2 anteriores intervalos definidos se suponía que la varianza es conocida, cuando no sucede esto la mejor alternativa para reemplazar a $\sigma^2$ es usando $S^2$, esto siempre y cuando el tamaño de muestra sea grande ($n>30$), para el caso de la diferencia de medias,
...
$$IC_{100(1-\alpha)}(\mu_1-\mu_2): \bar{X}_1-\bar{X}_2 \pm Z_{\alpha/2} \sqrt{\frac{S_1^2}{n_1}+\frac{S_2^2}{n_2}}$$
Ejercicio:
...
$$\bar{x}_1=18 \quad \sum_{m2} x_i=490 \quad S_1=7 \quad S_2=8.9$$
Calcular los siguientes intervalos de confianza al 90%:
  + Para cada media de las poblaciones $\mu_1, \mu_2$
  + Para la diferencia de medias $\mu_1-\mu_2$
  + Para la diferencia de medias $\mu_2-\mu_1$

Solución:

$$IC_{90\%}(\mu_1): 18\pm 1.64 \sqrt{\frac{7^2}{35}}=[16.06 \quad 19.94]$$
$$IC_{90\%}(\mu_1): 14\pm 1.64 \sqrt{\frac{8.9^2}{35}}=[11.53 \quad 16.47]$$
$$IC_{90\%}(\mu_1-\mu_2): [0.86 \quad 7.13]$$
$$IC_{90\%}(\mu_2-\mu_1): [-7.13 \quad -0.86]$$
...
```{r}
#95%
mean(s)+c(-1,1)*1.96*sqrt(var(s)/32)
#97
qnorm(0.03/2,lower.tail = F)
mean(s)+c(-1,1)*qnorm(0.03/2,lower.tail = F)*sqrt(var(s)/32)
```
...
Usando la tabla del libro guía, buscar el valor:
...
x
#solución, asumiendo que los datos provienen de una distribución normal
mean(x)
t95<-qt(0.05/2,n-1,lower.tail = F)
t98<-qt(0.02/2,n-1,lower.tail = F)
#95%
mean(x)+c(-1,1)*t95*sqrt(var(x)/n)
#98%
mean(x)+c(-1,1)*t98*sqrt(var(x)/n)
```
...
Para la diferencia de medias, pero con varianzas iguales $\sigma_1=\sigma_2$
...
$$v=\frac{\left(\frac{S_1^2}{n_1}+\frac{S^2_2}{n_2} \right)^2}{\frac{(S_1^2/n_1)^2}{n_1+1}+\frac{(S^2_2/n_2)^2}{n_2+1}}-2$$
...
En general, supongamos que los datos constan de $n$ pares $(X_{11}, X_{21}), (X_{12}, X_{22})...(X_{1n}, X_{2n})$, usualmente esto sucede cuando se trabaja con observaciones en 2 puntos distintos de tiempo. Definamos las diferencias como
$D_1=X_{11}-X_{21},D_2=X_{12}-X_{22},\ldots,D_n=X_{1n}-X_{2n}$,
...
qt(0.1/2,11,lower.tail = F)#ver página 779 del libro guía
...
p_re<-12/40
p_ab;p_ap; p_re
p_ab*100;p_ap*100; p_re*100
#intervalo de confianza
p_ab+c(-1,1)*1.96*sqrt(p_ab*(1-p_ab)/n) #abandonos
p_ap+c(-1,1)*1.96*sqrt(p_ap*(1-p_ap)/n) #aprobados
p_re+c(-1,1)*1.96*sqrt(p_re*(1-p_re)/n) #reprobados
#%
100*(p_ab+c(-1,1)*1.96*sqrt(p_ab*(1-p_ab)/n)) #abandonos
100*(p_ap+c(-1,1)*1.96*sqrt(p_ap*(1-p_ap)/n)) #aprobados
100*(p_re+c(-1,1)*1.96*sqrt(p_re*(1-p_re)/n))#reprobados
#personas
180*(p_ap+c(-1,1)*1.96*sqrt(p_ap*(1-p_ap)/n)) #aprobados
```
...
ceiling(tm_p(1.96,0.1,0.5))#epsilon 10%
ceiling(tm_p(1.96,0.01,0.5))#epsilon 1%
```

Nota: La muestra de 385 es valida para cualquier área, siempre y cuando la selección de la muestra se realiza de forma completamente aleatoria (**azar**) y provenir de una **listado** de la población objetivo.
...
$$IC_{95\%}(P_{II}-P_{I})=\frac{27}{45}-\frac{22}{45} \pm 1.96 * \sqrt{\frac{27/45(1-27/45)}{45}+\frac{22/45(1-22/45)}{45}}$$
$$=\frac{5}{45}\pm 0.2045\rightarrow [-0.093 \quad 0.315]  $$
...
$$IC_{100*(1-\alpha)}(\sigma^2): \left[\frac{(n-1){S}^2}{\chi^2_{\alpha/2,n-1}} \quad  \frac{(n-1){S}^2}{\chi^2_{1-\alpha/2,n-1}}\right]$$
...
$$IC_{95\%}(\sigma^2): \left[\frac{9*6.28}{\chi^2_{0.05/2,9}} \quad  \frac{9*6.28}{\chi^2_{1-0.05/2,9}}\right]=\left[\frac{56.52}{19.02} \quad  \frac{56.52}{2.7}\right]$$
$$=[2.97 \quad 20.93]$$
...
$$IC_{100*(1-\alpha)}\left(\frac{\sigma^2_1}{\sigma^2_2}\right): \left[\frac{{S}^2_1}{{S}^2_2} F_{1-\alpha/2,n_2-1,n_1-1} \quad \frac{{S}^2_1}{{S}^2_2} F_{\alpha/2,n_2-1,n_1-1}\right]$$
Para encontrar el valor de F en R. Suponer una muestra para ambas poblaciones de 13 y un nivel de confiabilidad del 95%
